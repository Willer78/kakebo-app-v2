...
// === Date helpers aggiuntivi (settimanale/trimestri) ===
...
const quarterRange = (year, q) => {
  const startMonth = (q-1)*3; // 0-index
...
function switchTab(which){
  tabIns.classList.remove("active"); tabRiep.classList.remove("active"); tabSett.classList.remove("active"); tabTri.classList.remove("active");
  secIns.classList.add("hidden"); secRiep.classList.add("hidden"); secSett.classList.add("hidden"); secTri.classList.add("hidden");
  ...
  else { tabTri.classList.add("active"); secTri.classList.remove("hidden"); aggiornaRiepilogoTrimestri(); }
}
...
// Popola macro e categorie in base al tipo (con gestione MUTUO disabilitato)
function refreshMacroETree(){
  const tipo = selTipo.value; // spesa | entrata | mutuo
  selMacro.innerHTML = "";
  selCat.innerHTML = "";
  ...
  if(tipo === "entrata"){
    ...
    o1.value = ""; o1.textContent = "— (non richiesto)";
    selMacro.appendChild(o1);
    const o2 = document.createElement("option");
    o2.value = ""; o2.textContent = "— (non richiesto)";
    selCat.appendChild(o2);
  } else {
...
// Submit form (con mutuo senza macro/categoria)
document.getElementById("form-movimento").addEventListener("submit", (e)=>{
  ...
  // mutuo non richiede macro/categoria
...
function aggiornaRiepilogo(){
  ...
  const totali = {}; // per macro (mutuo separato)
...
(function setDefaultMonthSett(){
  const d = new Date();
  inputMeseSett.value = `${d.getFullYear()}-${pad(d.getMonth()+1)}`;
})();
...
function aggiornaRiepilogoSettimanale(){
  ...
  const tbody = document.querySelector("#tabella-riepilogo-sett tbody");
  tbody.innerHTML = "";
  ...
  for(let cur = new Date(startMonday); cur < endExclusiveMonday; cur = addDays(cur, 7)){
    ...
    const tr = document.createElement("tr");
    tr.innerHTML = `
      <td>${formatRange(weekStart, weekEnd)}</td>
      <td>${fmtMoney(entrate)}</td>
      <td>${fmtMoney(speseSenzaMutuo)}</td>
...
(function setDefaultYear(){
  const d = new Date();
  inputAnnoTri.value = d.getFullYear();
})();

function aggiornaRiepilogoTrimestri(){
  const year = parseInt(inputAnnoTri.value, 10);
  if(isNaN(year)) return;

  const tbody = document.querySelector("#tabella-riepilogo-tri tbody");
  tbody.innerHTML = "";
  ...
  for(let q=1; q<=4; q++){
    ...
    for(const m of MOVIMENTI){
      ...
      if(d >= start && d <= end){
        if(m.tipo === "entrata") entrate += m.importo;
        else if(m.tipo === "mutuo"){ spese += m.importo; mutuo += m.importo; }
    ...
    tr.innerHTML = `
      ...
      <td>${periodStr}</td>
      <td>${fmtMoney(entrate)}</td>
      <td>${fmtMoney(spese)}</td>
      <td>${fmtMoney(mutuo)}</td>
      <td>${fmtMoney(saldo)}</td>
    `;
    tbody.appendChild(tr);
  }
