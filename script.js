/* Kakebo v3.8.3 — STABLE (offline-first, no cloud) */
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.getElementById("btn-backup-export")?.addEventListener("click", ()=>{
    const payload={version:"3.8.3", exportedAt:new Date().toISOString(), data:Array.isArray(MOVIMENTI)?MOVIMENTI:[]};
    const d=new Date(); const ym=`{0}-{1}`.replace("{0}",d.getFullYear()).replace("{1}",pad(d.getMonth()+1));
  ...
  function aggiornaRiepilogoTrimestri(){
    ...
    for(let q=1;q<=4;q++){
      ...
      const tr=document.createElement("tr"); tr.innerHTML=`<td>Q{q}</td><td>${periodStr}</td><td>${fmtMoney(entrate)}</td><td>${fmtMoney(spese)}</td><td>${fmtMoney(mutuo)}</td><td>${fmtMoney(saldo)}</td>`.replace("{q}",q); tbody.appendChild(tr);
    }
  }
