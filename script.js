...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function enableSelect(el){ if(!el) return; el.removeAttribute("disabled"); }
  function disableSelect(el){ if(!el) return; el.setAttribute("disabled","disabled"); }
  ...
  function refreshMacroETree(){
    ...
    refreshCategorie();
    // fallback se qualcosa va storto per timing
    if(!selMacro.options.length){
      setTimeout(()=>{ for(const m of macros){ const o=document.createElement("option"); o.value=m; o.textContent=m; selMacro.appendChild(o);} refreshCategorie(); },0);
    }
  }
  ...
  // Boot: precompila data e forza "spesa"
  inputData && (inputData.value = todayStr());
  ...
  document.getElementById("form-movimento")?.addEventListener("submit",(e)=>{
    e.preventDefault();
    const data=inputData?.value; const importo=parseFloat(inputImporto?.value||"");
    const tipo=selTipo?.value;
    ...
    MOVIMENTI.unshift(mov); salva(); renderTabella(); e.target.reset(); if(inputData) inputData.value=todayStr(); if(selTipo) selTipo.value="spesa"; refreshMacroETree(); aggiornaTutti();
  });

  document.getElementById("pulisci")?.addEventListener("click", ()=>{ document.getElementById("form-movimento")?.reset(); if(inputData) inputData.value=todayStr(); if(selTipo) selTipo.value="spesa"; refreshMacroETree(); });
  ...
  document.getElementById("btn-backup-export")?.addEventListener("click", ()=>{
    const payload={version:"3.8.4", exportedAt:new Date().toISOString(), data:Array.isArray(MOVIMENTI)?MOVIMENTI:[]};
  ...
  function aggiornaRiepilogo(){
    ...
    const ul=$("#totali-macro"); if(!ul) return; ul.innerHTML="";
  ...
  function aggiornaRiepilogoTrimestri(){
    ...
    for(let q=1;q<=4;q++){
      ...
      const periodStr=`${String(start.getDate()).padStart(2,"0")}/{0}–{1}/{2}`.replace("{0}",String(start.getMonth()+1).padStart(2,"0")).replace("{1}",String(end.getDate()).padStart(2,"0")).replace("{2}",String(end.getMonth()+1).padStart(2,"0"));
  ...
  function aggiornaDashboard(){
    ...
    const canvas=document.getElementById("chart-macro"); if(!canvas) return;
