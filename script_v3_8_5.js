/* Kakebo v3.8.5 — FULL cache-buster (no disable macro/categoria) */
document.addEventListener("DOMContentLoaded", () => {
  ...
  function populateMacroAndCat(){
    if(!selTipo||!selMacro||!selCat) return;
    const tipo=selTipo.value;
    selMacro.innerHTML=""; selCat.innerHTML="";
    if (tipo==="mutuo"){
      // nascondi macro/categoria, ma NON disabilitare
      wrapMacro?.classList.add("hidden");
      wrapCategoria?.classList.add("hidden");
      selMacro.required=false; selCat.required=false;
      return;
    } else {
      wrapMacro?.classList.remove("hidden");
      wrapCategoria?.classList.remove("hidden");
    ...
    if(!selMacro.options.length){
      setTimeout(()=>{ for(const m of macros){ const o=document.createElement("option"); o.value=m; o.textContent=m; selMacro.appendChild(o);} populateCategorie(); },0);
    }
  }
  ...
  populateMacroAndCat();
  selTipo?.addEventListener("change", populateMacroAndCat);
  selMacro?.addEventListener("change", populateCategorie);
  ...
  document.getElementById("form-movimento")?.addEventListener("submit",(e)=>{
    ...
    MOVIMENTI.unshift(mov); salva(); renderTabella(); e.target.reset(); if(inputData) inputData.value=todayStr(); if(selTipo) selTipo.value="spesa"; populateMacroAndCat(); aggiornaTutti();
  });

  document.getElementById("pulisci")?.addEventListener("click", ()=>{ document.getElementById("form-movimento")?.reset(); if(inputData) inputData.value=todayStr(); if(selTipo) selTipo.value="spesa"; populateMacroAndCat(); });
  ...
  // ===== Riepiloghi/Dashboard (come v3.8.4, omessi qui per brevità del commento) =====
  // --- (copio le funzioni già testate) ---
  const fmtMoney=n=>`€ ${Number(n||0).toFixed(2)}`;
  ...
  // ===== Export PDF/CSV e Dashboard identici a v3.8.4 =====
